...
ALL_EVENTS_2026 = {
    ...
    "March": {"gap": 6, "days": 31, "events": {3: ("Holika Dahan", "Purnima"), 4: ("Holi", "Colors"), 6: ("Sankashti Chaturthi", "Ganesh Pujan"), 15: ("Papmochani Ekadashi", "Sankranti"), 16: ("Pradosh Vrat (K)", "Vrat"), 17: ("Masik Shivaratri", "Vrat"), 19: ("Gudi Padwa", "Ghatasthapana"), 26: ("Ram Navami", "Janmotsav"), 29: ("Kamada Ekadashi", "Vrat"), 30: ("Pradosh Vrat (S)", "Vrat")}},
...
st.markdown("""
    ...
    .date-box { aspect-ratio: 1; border: 1px solid #f8f8f8; border-radius: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; transition: 0.2s; }
...
# --- 2. DASHBOARD ---
            ...
            for i, row in leaders.iterrows():
                ...
                st.markdown(f"""
                    <div style="background:{bg}; padding:15px; border-radius:15px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                        <div style="display:flex; align-items:center; gap:12px;"><span>{medal}</span><div><b>{row['Name']}</b><br><small>📍 {row['Location']}</small></div></div>
                        <div style="color:#FF4D00; font-weight:bold;">{int(row['Mala'])} कुल माला</div>
                    </div>
                """, unsafe_allow_html=True)
    ...
    if u_phone in ADMIN_NUMBERS:
        with st.sidebar:
            ...
            st.download_button("📥 Excel Download", data=csv, file_name=f'ram_seva_history.csv')
            st.divider()
